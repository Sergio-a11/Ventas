...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <asp:Label ID="Label1" runat="server" Font-Size="XX-Large" Text="VENTAS"></asp:Label>
...
<asp:SqlDataSource ID="ventas" runat="server" ConflictDetection="CompareAllValues" ConnectionString="<%$ ConnectionStrings:textilesConnectionString %>" 
    DeleteCommand="DELETE FROM [venta] WHERE [Id] = @original_Id AND (([fecha] = @original_fecha) OR ([fecha] IS NULL AND @original_fecha IS NULL)) AND (([cantidad] = @original_cantidad) OR ([cantidad] IS NULL AND @original_cantidad IS NULL)) AND (([id_cliente] = @original_id_cliente) OR ([id_cliente] IS NULL AND @original_id_cliente IS NULL)) AND (([id_producto] = @original_id_producto) OR ([id_producto] IS NULL AND @original_id_producto IS NULL)) AND (([observaciones] = @original_observaciones) OR ([observaciones] IS NULL AND @original_observaciones IS NULL))" 
    InsertCommand="IF EXISTS (SELECT producto.Id FROM producto WHERE @id_producto = producto.ID) IF EXISTS(SELECT DISTINCT producto.Id FROM producto LEFT JOIN compra on producto.Id=compra.id_producto WHERE ((select SUM (compra.cantidad) from compra where compra.id_producto=producto.Id)-ISNULL((select SUM (venta.cantidad) from venta where venta.id_producto=producto.Id),0))>0 AND producto.Id=@id_producto) INSERT INTO [venta] ([fecha], [cantidad], [id_cliente], [id_producto], [observaciones]) VALUES (@fecha, @cantidad, @id_cliente, @id_producto, @observaciones)" 
    OldValuesParameterFormatString="original_{0}" 
    SelectCommand="SELECT * FROM [venta]" 
    UpdateCommand="UPDATE [venta] SET [fecha] = @fecha, [cantidad] = @cantidad, [id_cliente] = @id_cliente, [id_producto] = @id_producto, [observaciones] = @observaciones WHERE [Id] = @original_Id AND (([fecha] = @original_fecha) OR ([fecha] IS NULL AND @original_fecha IS NULL)) AND (([cantidad] = @original_cantidad) OR ([cantidad] IS NULL AND @original_cantidad IS NULL)) AND (([id_cliente] = @original_id_cliente) OR ([id_cliente] IS NULL AND @original_id_cliente IS NULL)) AND (([id_producto] = @original_id_producto) OR ([id_producto] IS NULL AND @original_id_producto IS NULL)) AND (([observaciones] = @original_observaciones) OR ([observaciones] IS NULL AND @original_observaciones IS NULL))">
    <DeleteParameters>
