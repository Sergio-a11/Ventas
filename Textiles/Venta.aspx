...
<asp:SqlDataSource ID="ventas" runat="server" ConflictDetection="CompareAllValues" ConnectionString="<%$ ConnectionStrings:textilesConnectionString %>" DeleteCommand="DELETE FROM [venta] WHERE [Id] = @original_Id AND (([fecha] = @original_fecha) OR ([fecha] IS NULL AND @original_fecha IS NULL)) AND (([cantidad] = @original_cantidad) OR ([cantidad] IS NULL AND @original_cantidad IS NULL)) AND (([id_cliente] = @original_id_cliente) OR ([id_cliente] IS NULL AND @original_id_cliente IS NULL)) AND (([id_producto] = @original_id_producto) OR ([id_producto] IS NULL AND @original_id_producto IS NULL)) AND (([observaciones] = @original_observaciones) OR ([observaciones] IS NULL AND @original_observaciones IS NULL))" InsertCommand="IF EXISTS (SELECT producto.Id FROM producto WHERE @id_producto = producto.ID) IF EXISTS(SELECT compra.cantidad-venta.cantidad  as CantidadTotal FROM venta join producto on venta.id_producto = @id_producto join compra on compra.id_producto = @id_producto WHERE (select SUM (compra.cantidad) from compra where venta.id_producto=compra.id_producto)-(select SUM (venta.cantidad) from venta where venta.id_producto=compra.id_producto)>0) INSERT INTO [venta] ([fecha], [cantidad], [id_cliente], [id_producto], [observaciones]) VALUES (@fecha, @cantidad, @id_cliente, @id_producto, @observaciones)" OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT * FROM [venta]" UpdateCommand="UPDATE [venta] SET [fecha] = @fecha, [cantidad] = @cantidad, [id_cliente] = @id_cliente, [id_producto] = @id_producto, [observaciones] = @observaciones WHERE [Id] = @original_Id AND (([fecha] = @original_fecha) OR ([fecha] IS NULL AND @original_fecha IS NULL)) AND (([cantidad] = @original_cantidad) OR ([cantidad] IS NULL AND @original_cantidad IS NULL)) AND (([id_cliente] = @original_id_cliente) OR ([id_cliente] IS NULL AND @original_id_cliente IS NULL)) AND (([id_producto] = @original_id_producto) OR ([id_producto] IS NULL AND @original_id_producto IS NULL)) AND (([observaciones] = @original_observaciones) OR ([observaciones] IS NULL AND @original_observaciones IS NULL))">
    <DeleteParameters>
